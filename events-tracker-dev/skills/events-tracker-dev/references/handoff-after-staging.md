...
Recipe for the dev's hand-off: an own Impl Issue has reached `ENG` `In Staging` (merged + deployed to staging), route it to the right review phase — `Design Review` if it carries the `UI` label, else `QA` — with an @-mention to the reviewer. Loaded on demand from `events-tracker-dev` → `SKILL.md` → § References pointer. Entry-point trigger: "выложил на стейджинг", "отдай на ревью / QA", "hand off ENG-N", "it's on staging, who reviews".
...
The work is merged and on staging — manually transitioned to `In Staging` for now (once the GitHub integration is on, the merge does this; base § Statuses → `ENG`). Before you hand off, you've already self-tested on staging as if no review phase existed (delivery-flow § 3) — the review phase validates, it doesn't first-detect. Now route it.
...
The `UI` label is set by the PM at creation when the change touches the interface (base § Labels → `ENG`; Invariant 3). If you believe it's wrong — the change doesn't actually touch the UI, or it does and `UI` is missing — see § Disputing the `UI` label below before routing.
...
Don't set any label here — `Design approved` is the designer's (base § Labels → `ENG`). Assignee stays you — no `assignee` field (Invariant 1).
...
- `ENG-N` → `Design Review` (designer @-mentioned) **or** → `QA` (QA @-mentioned), with the staging link included.
