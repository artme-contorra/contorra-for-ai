...
description: >
  ...
  tracker (teams `ENG` Events Engineering and `DES` Events Design) at
...
| Dev (web) | 2 | Linear assignee for Impl Issues in `ENG` and `lead` on Projects. |
...
| Key | Name | ID | Purpose |
|---|---|---|---|
| `ENG` | Events Engineering | `23ad419d-cc6f-423d-a7f7-18cccb2833cc` | Impl Issues (web). Weekly cycles. |
| `DES` | Events Design | `3088ba1c-456d-4357-af55-ec4b288480bc` | Design Issues. No cycles. |

Issue identifiers: `ENG-N`, `DES-N`. URL pattern: `https://linear.app/contorra/issue/<KEY>-<N>/<slug>`. Workspace name in URLs is `contorra`.
...
- **Cross-team** (both `ENG` + `DES`): `[Events] Test` (sandbox), `Automated testing` (Stability), `Additional formats`.
- **`ENG` only:** `Customizable team names`, `Security` (Stability), `CI & release pipeline` (Stability), `Error monitoring` (Stability), `Session replay` (UX), `Backend refactor` (AI Platform).
...
- **Project** — time-bounded deliverable. **Cross-team container** — attach `ENG` + `DES` when the work spans design + impl. Threshold from Issue: work spans weeks and naturally splits across roles. Lead = the responsible dev (= future Impl Issue assignee). Description required (one paragraph max).
- **Issue** — single assignee, discrete unit.
  - *Inside Project:* role-split into Design Issue (`DES`, assignee = designer) + Impl Issue (`ENG`, assignee = dev). Description minimal or skipped — context lives in Project description.
...
**`DES`** — 7-status set (no `Triage`, no PR / staging / QA gates):
...
No Type group on `DES`. No `UI` (design issues are by definition UI).
...
- `state` accepts type or name: by type `state: "started"` covers `In Progress` + `In Review` + `In Staging` + `Design Review` + `QA` (5 statuses) on `ENG`; by name use the exact status string (`"Design Review"`, `"In Staging"`).
- `assignee` — pass the `displayName`, name, email, or UUID (resolve via § Map → Team or `list_users`). `"me"` also valid.
- `priority`: `1` Urgent, `2` High, `3` Medium, `4` Low.
- `cycle` — by name, number, or ID. For "current cycle on ENG" first fetch via `list_cycles teamId:<UUID> type:"current"` (see gotcha below).
...
- `labels: ["bug"]` (or `feature` / `chore` / `tech-debt`) on `ENG`. Bare label name, no `Type/` prefix even though Linear renders the group.
- Add `"UI"` to the labels array when the Impl Issue changes the UI.
- `blocks: ["ENG-12"]` / `blockedBy: ["DES-7"]` for cross-team dependency.

### `save_issue` — update

- Pass `id` (e.g. `"ENG-3"`). Omitting `id` creates a new issue.
...
- `issueId: "ENG-12"` + `body` for a top-level thread.
...
- `ENG`: `23ad419d-cc6f-423d-a7f7-18cccb2833cc`
- `DES`: `3088ba1c-456d-4357-af55-ec4b288480bc` (cycles not used on `DES`; expected to return `[]`).
